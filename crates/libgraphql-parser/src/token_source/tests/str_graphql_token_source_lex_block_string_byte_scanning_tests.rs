...
//!
//! `lex_block_string()` byte-scans for sentinel bytes (`"`, `\`)
//! instead of per-character `peek_char()` / `consume()`.
//! Non-sentinel bytes are skipped with a single `i += 1`.
//! Line/column resolution is deferred to SourceMap.
//!
...
fn multiline_block_string_positions() {
    ...
    let input = "\"\"\"line1\nline2\nline3\"\"\" name";
    let (tokens, source_map) =
        StrGraphQLTokenSource::new(input)
            .collect_with_source_map();
    // Tokens: StringValue, Name, Eof
    assert_eq!(tokens.len(), 3);
...
///
/// The SourceMap resolves byte offsets to character-based columns.
///
/// Written by Claude Code, reviewed by a human.
#[test]
