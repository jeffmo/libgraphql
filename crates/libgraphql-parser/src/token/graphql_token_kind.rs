...
fn parse_block_string(
    ...
    // --- Pass 1: Compute common indent, first/last non-blank
    //     line indices, and line count -----------------------
    //
    ...
    // Edge case: if the block string is a single trailing
    // newline (e.g. `"""\n"""`), lines() yields one empty
    // line which is blank, so first_non_blank would be None
    // and we'd have returned early above.
    let _ = line_count;
