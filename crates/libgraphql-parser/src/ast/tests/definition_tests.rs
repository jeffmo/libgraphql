...
/// Verify `Definition::SchemaDefinition` variant
/// delegates `append_source` correctly.
///
/// Relevant spec section:
/// https://spec.graphql.org/September2025/#sec-Document
...
/// Written by Claude Code, reviewed by a human.
#[test]
fn definition_schema_definition_source_slice() {
    let source =
        "schema { query: Query }";
    let def = Definition::SchemaDefinition(
        SchemaDefinition {
