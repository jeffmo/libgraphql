...
#[derive(Debug)]
pub struct GoldenTestResults {
    pub results: Vec<GoldenTestResult>,
}

impl GoldenTestResults {
    pub fn new() -> Self {
        Self {
            results: Vec::new(),
        }
    ...
    pub fn failure_report(&self) -> String {
        ...
        format!("{failures_len} of {results_len} golden tests failed:\n\n{failures_text}")
    }
...
fn format_detailed_failure(result: &GoldenTestResult) -> String {
...
fn test_valid_schema(test_case: &GoldenTestCase) -> GoldenTestResult {
...
fn test_invalid_schema(test_case: &GoldenTestCase) -> GoldenTestResult {
    let name = &test_case.name;
    let test_name = format!("{name}/schema");
