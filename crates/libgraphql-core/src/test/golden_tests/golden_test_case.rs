...
#[derive(Debug, Clone)]
pub struct GoldenOperationTestCase {
    pub path: PathBuf,
    pub should_be_valid: bool,
    pub expected_errors: Vec<String>,
}

impl GoldenOperationTestCase {
...
/// Represents a complete golden test case (schema + operations)
#[derive(Debug, Clone)]
...
impl GoldenTestCase {
    /// Discovers all golden test cases from the fixtures directory
        ...
        entries
            .filter_map(|entry| {
                ...
                let schema_expected_errors = schema_paths
                    .iter()
                    .flat_map(|p| GoldenOperationTestCase::parse_expected_errors(p))
                    .collect();
