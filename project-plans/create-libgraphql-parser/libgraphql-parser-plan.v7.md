...
3. Create `/crates/libgraphql-parser/src/source_position.rs`:
   ...
   /// - `col`: UTF-8 byte offset within the current line (0-based)
   /// - `byte_offset`: UTF-8 byte offset within the whole document (0-based)
   ///
   /// **Important:** `col` is a byte offset, NOT a character/codepoint offset.
   /// For ASCII text, `col` equals the character position. For text containing
   /// multi-byte UTF-8 characters, `col` advances by `char.len_utf8()` bytes
   /// per character.
   ///
   /// This matches common programming conventions (array indexing, string slicing)
   /// and makes `&source[byte_offset..]` and `&line[col..]` work correctly.
   #[derive(Clone, Debug, Eq, PartialEq)]
   pub struct SourcePosition {
       /// Line number (0-based: first line is 0)
       line: usize,
       /// UTF-8 byte offset within current line (0-based: first byte is 0)
       col: usize,
       /// UTF-8 byte offset from start of document (0-based: first byte is 0)
       byte_offset: usize,
   }
   ...
   impl SourcePosition {
       ...
       /// - `col`: 0-based UTF-8 byte offset within current line
       /// - `byte_offset`: 0-based UTF-8 byte offset from document start
       pub fn new(line: usize, col: usize, byte_offset: usize) -> Self { ... }

       /// Returns the 0-based line number.
       pub fn line(&self) -> usize { self.line }
       /// Returns the 0-based UTF-8 byte offset within the current line.
       pub fn col(&self) -> usize { self.col }
       /// Returns the 0-based UTF-8 byte offset from document start.
       pub fn byte_offset(&self) -> usize { self.byte_offset }

       /// Advance position by a single character.
       /// Handles newlines (resets col, increments line) and multi-byte UTF-8.
       pub fn advance_by_char(&mut self, ch: char) {
           if ch == '\n' {
               self.line += 1;
               self.col = 0;
           } else {
...
1. Create `/crates/libgraphql-parser/src/token/graphql_token.rs` with trivia-aware structure:
   ...
   pub enum GraphQLTokenKind {
       ...
       Name(String),
       /// Raw source text of integer literal (e.g., "-123", "0")
       IntValue(String),
       /// Raw source text of float literal (e.g., "1.23e-4", "0.5")
       FloatValue(String),
       ...
       pub fn cook_string_value(&self) -> Option<Result<String, CookGraphQLStringError>> {
           match self {
               GraphQLTokenKind::StringValue(raw) => {
                   ...
                   // Implementation handles both single-line and block strings
                   Some(cook_graphql_string(raw))
               }
               _ => None,
           }
       }
...
2. Update `GraphQLTokenStream` to be generic over `GraphQLTokenSource`:
   ...
   /// Compaction happens lazily, NOT inside `consume()`.
   ///
   ...
   impl<S: GraphQLTokenSource> GraphQLTokenStream<S> {
       ...
       /// Call this periodically (e.g., after parsing each definition) to
       /// prevent unbounded buffer growth. NOT called inside consume().
...
3. Implement internal buffering:
   ...
   - Compaction is lazy - called by parser at natural boundaries, NOT inside `consume()`
