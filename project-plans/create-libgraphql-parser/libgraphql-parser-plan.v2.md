...
- Create new `SourcePosition` struct with `col`, `line`, `offset` fields (no dependency on `libgraphql-core`)
...
- `RustToGraphQLTokenSource` stays in `libgraphql-macros` (implements `GraphQLTokenSource` trait from parser crate)
...
- `/crates/libgraphql-parser/src/parser_location.rs` - Location abstraction
- `/crates/libgraphql-macros/src/rust_to_graphql_token_source.rs` - Proc-macro token source (implements trait)
...
3. Add dependencies to `/crates/libgraphql-parser/Cargo.toml` (using workspace dependencies):
   ```toml
   [dependencies]
...
4. Create `/crates/libgraphql-parser/src/parser_location.rs`:
   ```rust
   use crate::ast::AstPos;
   ...
   pub enum ParserLocation {
       /// Location from proc-macro token stream
       Span(Span),
       /// Location from string-based parsing
       SourcePosition(SourcePosition),
   }

   impl ParserLocation {
       pub fn to_ast_pos(&self) -> AstPos { ... }
       pub fn to_source_position(&self) -> Option<&SourcePosition> { ... }
   }
   ```
...
- One struct per file: `source_position.rs` and `parser_location.rs`
...
2. Create `/crates/libgraphql-parser/src/graphql_token_source.rs` with trait definition:
   ```rust
   use crate::graphql_token::GraphQLToken;
   use crate::parser_location::ParserLocation;

   /// Trait for sources that produce GraphQL tokens.
   ///
   /// This trait enables extensibility - third parties can implement custom
   /// token sources (e.g., from different input formats or with custom preprocessing).
   ///
   /// # Implementing
   ///
   /// Implementors should produce a stream of tokens with their associated locations.
   /// The token stream should handle whitespace/comma skipping internally (these are
   /// "ignored tokens" per the GraphQL spec).
   pub trait GraphQLTokenSource: Iterator<Item = (GraphQLToken, ParserLocation)> {
       /// Returns true if there are no more tokens to consume
       fn is_at_end(&mut self) -> bool;

       /// Peek at the next token without consuming it
       fn peek(&mut self) -> Option<&(GraphQLToken, ParserLocation)>;

       /// Peek at the nth token ahead (0-indexed)
       fn peek_nth(&mut self, n: usize) -> Option<&(GraphQLToken, ParserLocation)>;
   }
   ```

3. Update all imports in macros crate to use new location
...
2. Update `GraphQLTokenStream` to be generic over `GraphQLTokenSource`:
   ...
   pub struct GraphQLTokenStream<S: GraphQLTokenSource> {
       source: S,
       buffer: VecDeque<(GraphQLToken, ParserLocation)>,
       current_location: Option<ParserLocation>,
   }

   impl<S: GraphQLTokenSource> GraphQLTokenStream<S> {
       pub fn new(source: S) -> Self { ... }
       pub fn peek(&mut self) -> Option<&(GraphQLToken, ParserLocation)> { ... }
       pub fn peek_nth(&mut self, n: usize) -> Option<&(GraphQLToken, ParserLocation)> { ... }
       pub fn next(&mut self) -> Option<(GraphQLToken, ParserLocation)> { ... }
       pub fn current_location(&self) -> &ParserLocation { ... }
...
### Step 1.4: Update RustToGraphQLTokenSource to Implement Trait
**Outcome:** `RustToGraphQLTokenSource` in macros crate implements `GraphQLTokenSource` trait

**Tasks:**
1. Keep `/crates/libgraphql-macros/src/rust_to_graphql_token_adapter.rs` in place but rename to `rust_to_graphql_token_source.rs`
...
4. Update `RustToGraphQLTokenSource` to implement `GraphQLTokenSource` trait:
   ```rust
   use libgraphql_parser::graphql_token::GraphQLToken;
   use libgraphql_parser::graphql_token_source::GraphQLTokenSource;
   use libgraphql_parser::parser_location::ParserLocation;

   impl Iterator for RustToGraphQLTokenSource {
       type Item = (GraphQLToken, ParserLocation);
       fn next(&mut self) -> Option<Self::Item> { ... }
   }

   impl GraphQLTokenSource for RustToGraphQLTokenSource {
       fn is_at_end(&mut self) -> bool { ... }
       fn peek(&mut self) -> Option<&(GraphQLToken, ParserLocation)> { ... }
       fn peek_nth(&mut self, n: usize) -> Option<&(GraphQLToken, ParserLocation)> { ... }
   }
...
5. Update to return `ParserLocation::Span(span)` instead of raw `Span`

6. Update to emit new explicit `GraphQLToken` variants (e.g., `GraphQLToken::BraceOpen` instead of `GraphQLToken::Punctuator("{".to_string())`)
...
**Considerations:**
- `RustToGraphQLTokenSource` stays in `libgraphql-macros` (where it belongs - it's proc-macro specific)
...
1. Create `/crates/libgraphql-parser/src/` module structure:
   ...
   ```
   Note: `RustToGraphQLTokenSource` stays in `libgraphql-macros` (Step 1.4)
