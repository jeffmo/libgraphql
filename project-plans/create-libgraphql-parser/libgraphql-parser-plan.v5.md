...
3. Create `/crates/libgraphql-parser/src/source_position.rs`:
   ...
   impl SourcePosition {
       ...
       pub fn from_span(span: &proc_macro2::Span) -> Self {
           ...
           Self::new(start.line, start.column, span.byte_range().start)
       }
   }
   ```
...
- `byte_offset` field enables efficient error reporting with source snippets (available from `Span::byte_range().start`)
...
1. Create `/crates/libgraphql-parser/src/graphql_token.rs` with trivia-aware structure:
   ...
   ///
   /// TODO: Profile whether `Vec<GraphQLTriviaToken>` allocation per token
   /// is a performance concern. If so, consider `SmallVec` optimization.
   #[derive(Clone, Debug, PartialEq)]
   pub enum GraphQLToken {
       Token {
           kind: GraphQLTokenKind,
           preceding_trivia: Vec<GraphQLTriviaToken>,
           span: GraphQLTokenSpan,
       },
       Error {
           message: String,
           /// TODO: Explore replacing `Vec<String>` with a richer `Diagnostic`
   ...
   /// The kind of a valid GraphQL token.
   ///
...
4. Update `RustMacroGraphQLTokenSource` to implement `GraphQLTokenSource` trait:
   ...
   impl Iterator for RustMacroGraphQLTokenSource {
       ...
       fn next(&mut self) -> Option<Self::Item> {
           // Accumulate trivia (comments from Rust // syntax, commas)
...
5. Implement trivia accumulation:
...
6. Convert `proc_macro2::Span` to `GraphQLTokenSpan`:
   - Use `SourcePosition::from_span()` for start position
   - Use span end for end position

7. Update to emit `GraphQLTokenKind` variants (e.g., `GraphQLTokenKind::CurlyBraceOpen`)
...
3. **Differential testing against `graphql_parser`**:
...
5. Add regression tests for discovered bugs
